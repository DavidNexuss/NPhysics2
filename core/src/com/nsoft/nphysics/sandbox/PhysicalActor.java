...
/**
 * Classe encarregada de definir un cos abstracte que nom�s consta d'una forma i un pes a la fase
 * Sandbox.
 * Necessita una classe Gen�rica que defineixi quin ha de ser el procediment per crear la seva representaci� a la simulaci�
 * @see Definici� Circular{@link CircleDefinition}
 * @see Definici� poligon tancat {@link PolygonDefinition}
 * 
 * Hereda Group per poder passar la crida de dibuix i la de act als seus fills, ObjectChildrens. A part tamb� hereda les funcions de renderitzaci�
 * Listeners i Animacions d'Actor
 * -Implementa {@link Form} per poder definir un formulari i modificar propietats dins l'aplicaci�.
 ...
 * 	de forma abstracta la seva participaci�
 * -Implementa {@link Removeable} per poder ser seleccionat com a objecte candidat a ser destruit de la simulaci� ja que inclou un procediment per fer-ho
 * @author Usuari
 *
 * @param <D>
 */
public abstract class PhysicalActor<D extends ObjectDefinition> extends Group implements Form,Handler,ClickIn,Draggable,Parent<Point>,Removeable,Say{
	...
	//Colors ja definits de forma est�tica per evitar ser instantiats dins del bucle de renderitzat i augmentar el rendiment.
	...
	private SelectHandle handler; //Objecte encarregat de manejar solicituds de selecci� del handler superior.
	...
	D definition; //Definici� del objecte per a la simulaci�
	...
	public void end() {
		...
		if(isEnded) throw new IllegalStateException(); //Necessari per controlar que aquesta funci� nom�s es pugui cridar un sol cop
		isEnded = true;
	...
	//Funci� necess�ria per poder rotar el cos entorn a un eix
	...
	//S'implementa per poder llan�ar el opcionari
